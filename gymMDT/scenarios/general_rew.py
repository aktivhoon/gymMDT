...
class GeneralRewardScenario(BaseScenario):
...
class GeneralRewardEnv(MDTEnv):
    def __init__(self, code, n_blocks=20):
        assert len(code) == 16 + n_blocks, f'code must be a list of {4 + n_blocks} numbers (16 reward codes + {n_blocks} task codes)'
        assert all(0 <= x <= 3 for x in code[:16]), 'first 16 numbers of code must be between 0 and 3'
        assert all(0 <= x <= 11 for x in code[16:]), f'last {n_blocks} numbers of code must be between 0 and 11'
        
        scenario = GeneralRewardScenario()
        world = scenario.make_world(code, n_blocks)
        super(GeneralRewardEnv, self).__init__(world, reset_callback=scenario.reset_world, 
